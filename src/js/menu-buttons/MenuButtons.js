...
export default class MenuButtons extends Component {
    ...
    render() {
        ...
        const { btnTheme, backgroundColor, boxShadow } = Theme[this.context];
        return (
            <nav className="descNav" style={{ backgroundColor }}>
                ...
                {options && <div className="optionPage" style={{ boxShadow }}>
                    ...
                    <div className="option" onClick={() => this.props.changeThemeHandler_('magentaTheme')}>magenta</div>
                    <div className="option" onClick={() => this.props.changeThemeHandler_('yellowTheme')}>yellow</div>
                </div>}
            </nav>
        )
    }
